...
| **口播处理** | 角色嘴唇同步说话（引号嵌入画面） | 画外音 V.O.（末尾标记，不嵌入画面动作） |
...
- **镜头语言**：始终以 `POV first-person` / `第一人称视角` 为基底，叠加具体运镜
...
POV 模式下角色不出脸，口播台词使用**画外音**方式：
- **Seedance 2.0**：在 Prompt 末尾添加 `画外音旁白：角色用轻松的语气说 "…"`，不嵌入画面动作描述中
- **Veo 3.1 / Sora 2**：在 Prompt 末尾使用 `V.O. (off-screen): "…"` 标记，配合 `Audio:` 指令
...
- **多模态参考（References）的极简准则**：
  ...
  - **音频处理**：完全舍弃 `@Audio` 标签参考！将角色的英文口播台词直接用引号嵌入 Prompt 画面描述中，在 Prompt 末尾用中文描述背景音乐风格和环境音效。
  ...
  - **示例对比**：
    - ❌ **错误（废话太多）**：镜头推进，绝望的机修工在昏暗的车库满头大汗，像 @视频1 那样吃力使用一把伤痕累累的钝折刀 @图片1，在黑色厚橡胶管 @图片2 上拼命割，却切不开，他绝望地叹息说...
    - ✅ **正确（精简硬核）**：第一人称 POV 视角，暖焦黄色强光台灯下。从 @图片1 口袋中拔出 @图片2 进行展示。用 @视频1 的动作在 @图片3 橡胶管上拉锯。没有切痕。修车工长舒一口气说道 "This saves my sanity."
...
**Segment [N] ([合并后的时间段])**:
[镜头视角]，[1句精简打光/环境]。从 @图片[N] [动作描述] 取出 @图片[N] [物品]。拔下 @图片[N] [部位]，呈现出 @图片[N] 的样子。用 @视频[N] 的动作在 @图片[N] 的[物品表面]进行[动作]。[1句干脆的结果描述，如"金属剧烈切割，没有火星特效"]。紧跟在 @图片[N] [表面]极度顺滑、用 @视频[N] 的动作进行[动作]。[角色极简状态，如"紧绷神经松弛"]，说道 "口播台词内容。"
...
- **美妆/护肤品真人实测 (Skin Care Test)**：
  - `浴室洗手台前，暖黄灯光。拿起 @图片1 精华液。像 @视频1 的手势在 @图片2 脸颊上涂抹。皮肤纹理真实可见。惊喜地说 "Wait, this texture is actually insane."`
- **家居/生活好物分享 (Home Gadget)**：
  - `第一人称 POV 视角，凌乱厨房台面。手持 @图片1 压蒜器。像 @视频1 那样用力压爆 @图片2 的大蒜。对焦起伏。轻松地说 "Bro, where has this been my whole life?"`

**POV 模式实战提示词模板参考**（`视角：POV` 时触发）：
- **工具类 POV 开箱使用 (Tool Unboxing & Demo)**：
  - `第一人称 POV 手持视角，白皙男手，黑色运动表。从包裹中取出 @图片1 放在 @图片2 木质台面。拿起 @图片3 钝刀卡上去，像 @视频1 的节奏推拉五下。停顿。拿刀像 @视频2 下切 @图片4 的番茄，切面平整。画外音男声说 "Dude, five strokes. That's literally it."`
- **电子产品 POV 桌面体验 (Tech Desk Setup)**：
  - `第一人称 POV 手持视角，戴银戒指的手。从包装盒取出 @图片1 无线充放在桌面。将手机放在 @图片1 上，屏幕亮起充电动画。竖起大拇指入镜。画外音说 "No cable, no waiting, it just works."`
...
| 4 | **音频/口播指令** | 每段 Prompt 是否在末尾包含背景音乐和环境音效描述？角色口播台词是否用引号嵌入画面描述中？ |
