...
```text
# ✅ 正确写法——简洁直接
手稳定有力地用图2的刀在图1磨刀石的粗磨面上磨了一下，用图3的磨刀姿势。
...
> [!IMPORTANT]
> 参考图必须使用**真实产品/动作实拍照片**，避免 AI 凭空想象产品外观导致幻觉。每条提示词参考图控制在 **2-4 张**。
...
第一格：[用图N的XX做什么，画面描述]
第二格：[画面描述]
第三格：[用图N的XX，画面描述。没有XX，没有XX（产品物理限制）]
第四格：[用图N的XX在图N的XX上做什么，用图N的姿势。没有XX，没有XX（产品物理限制）]
第五格：[画面描述]
第六格：[画面描述]
...
```text
第一人称 POV 视角，[手部/角色描述]。[用图N的XX做什么，核心画面描述]。[没有XX（产品物理限制，仅涉及产品操作时添加）]。iPhone 15 Pro 手机拍摄画质，9:16 竖屏。
```
...
根据产品的真实物理特性，将负面约束**直接写进提示词正文**，使用自然语言"没有XX"格式。
...
# ❌ 错误写法——单独放在 Negative 段
**Negative**: 火花、水、大块碎片
```

> [!IMPORTANT]
> 只在**涉及产品操作的分镜**中内嵌负面约束。不操作产品的分镜（如钝刀切番茄、CTA 指向链接）无需添加。
...
| 7 | **敏感内容** | 被模型风控拦截 | 参照 AI视频提示词生成skill 中的四大类违禁词规则 |
