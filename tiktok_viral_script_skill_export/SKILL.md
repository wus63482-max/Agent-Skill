...
## 输出落盘规则 (Output Saving Rules)

当用户要求把生成结果保存、导出或落盘到本地文件时，必须遵守以下规则：

1. 所有**新生成的视频脚本**一律保存到 `tiktok_viral_script_skill_export/output/YYYY-MM-DD/`。
2. `YYYY-MM-DD` 默认使用当前本地日期；如果用户明确指定日期，则按用户指定日期执行。
3. 如果对应日期文件夹不存在，先创建文件夹；**不要**把当天新文件直接放在 `output/` 根目录。
4. 如果同一轮还要继续生成视频提示词，脚本文件名确定后要保持稳定，方便下游 skill 直接引用同名源脚本。
...
- [示例：Madebut 千牛电动扳手案例](examples/madebut_impact_wrench_case.md)
...
- 如果本次任务包含“写入文件”或“导出 Markdown”，默认当天成品应位于 `output/YYYY-MM-DD/`，而不是 `output/` 根目录。
