...
### Step 4：自动转换为纳米香蕉 2 分镜图 (Auto Storyboard Image Generation)

> [!IMPORTANT]
> 前三步完成后且**通过了自检**，你必须立刻**自动、无缝执行**此步骤，无需等待用户指令。

**目的**：将 Step 2 生成的分镜脚本自动转换为**纳米香蕉 2（Nano Banana 2）图片生成模型**的单格分镜图提示词，供视觉预审和创意验证。

**规则**：

| 要求 | 说明 |
|------|------|
| 角色切换 | 自动读取 `../纳米香蕉2图片提示词生成skill/SKILL.md`，**切换为该 Skill 角色**并严格遵循其所有规则 |
| 输入 | 基于 Step 2 生成的 **2 套分镜脚本** |
| 风格 | 统一使用**写实 UGC 手机拍摄风格**，**POV 第一人称视角**。**【🚨POV 视角核心法则】**：绝对不可出现主语的表面神态、情绪修饰语或全身动作描述（如"极度疲惫的主妇"、"满头大汗的父亲"）。必须直接从画面视角的所见出发，专注于手部动作、被持握的物品、以及镜头本身的状态（如"镜头因用力而急躁地晃动"）。 |
| 输出数量 | 每套脚本必须输出 **精确的 6 张分镜图提示词**（包含不同核心景别的动作拆解，如 Hook动作/Hook结果/Solve动作/Solve效果/Payoff展示/CTA收尾），2 套脚本共计 **12 张**分镜图。 |
| 画面禁止项 | **【🚨核心铁律】**所有生成的提示词中必须强制加入负面限制：**画面中严禁出现任何形式的文字、字幕、标题、logo或文本**，人物服装和产品包装上必须是纯净无字的。 |

**映射逻辑**：

将 Step 2 中的每个分镜段落（Hook、Agitate、Solve、Payoff、CTA）的关键画面描述，提取并转换为单张提示词的主体。

**提示词生成规范（POV 公式速查）**：

必须套用纳米香蕉 2 最新 POV 官方公式构建中文提示词：
`[手部描述+产品] + [使用动作] + [场景] + [自然光影] + [手机+9:16+POV]`

3. **【🚨核心铁律：参考图必选范围】**：参考图**必须严格限制在此四个核心要素范围内：刀具 (Knife)、产品实体 (Product)、场景/环境 (Scene)、产品使用方式 (Usage Method/Action)**。绝对禁止使用参考图来定义其他无关紧要的事物（如无关紧要的小道具、切下的碎片等）。每一处画面描写必须用 `图N的XX` 开头定义。
4. **负面提示词（物理与文字限制）**：从 `pain_points/[产品].md` 提取物理限制，并**必须用纯中文表达**无文字限制（如`绝对禁止画面中出现任何文字、字幕或logo`）。不再使用长串的英文字段。
5. **UGC 风格关键词（纯中文呈现）**：必须将 UGC 风格词中文化，并融入公式后两个部分（光影与画质），例如 `iPhone 15 Pro手持轻微摇晃抓拍`、`原生UGC未修饰画质`、`真实的皮肤毛孔细节`、`自然窗户光`、`竖屏 9:16`。
6. **每条提示词 60-120 词**，全程使用中文。
7. **禁止**：使用任何英文翻译出来的模糊美学词（如完美构图）、专业摄影词、完美主义词、AI 机械词。

**输出格式**：

每次单格分镜单独建立标题，并将提示词放入 code block 方便直接复制（不加 `Prompt:` 标签）：

<!-- 📋 COPYABLE OUTPUT: 单格分镜 -->
```text
### Script 1 纳米香蕉 2 分镜图
...
<!-- 📋 COPYABLE OUTPUT -->
```text
POV第一人称视角，仅可见一双手，握着图一的[道具/产品]执行[动作行为]处理图二的[目标对象]。场景在[具体的真实生活场景]。[自然光影/灯光说明]。竖屏 9:16，iPhone 15 Pro手持随手抓拍，轻微摇晃，真实的皮肤纹理与瑕疵，原生UGC低质感。[物理限制说明]。绝对禁止画面中出现任何形式的文字、字母、字幕或品牌logo。
...
> [!TIP]
> **可复制版本**：每套分镜图提示词输出完成后，额外提供一个 code block 包裹的**纯净可复制版本**（仅包含提示词纯文本，不需要输出 Image Config 等参数，方便用户直接复制）。

---

### Step 5：自动执行 AI 视频提示词转换 (Auto-Prompt Generation)

> [!IMPORTANT]
> Step 4 完成后，你必须立刻**自动、无缝执行**此步骤，无需等待用户指令。

**规则**：

| 要求 | 说明 |
|------|------|
| 角色切换 | 自动读取 `../AI视频提示词生成skill/SKILL.md`，**切换为该 Agent 角色**并严格遵循其所有规则 |
| 输入 | 基于上面生成的 2 套脚本 |
| 引擎 | **Seedance 2.0**（或用户指定的替代引擎） |
| 片段原则 | 必须采用**「少量片段原则」** |
| References 规范 | 必须采用 `@Video1`, `@Image1`，并遵循**极简公式**。<br><br>**【🚨绝对不可违背的无废话铁律】**：<br>1. **公式化组合**：`状态/视角介绍 + 动作源(@图片) + 动作类型(@视频) + 作用对象(@图片) + 结果态(@图片)`<br>2. **剔除冗余**：绝对不要加复杂的场景描写（"满是机油味的车库"）、情绪描写（"吃力地"、"绝望地"）。提示词就像无情的动作执行手册。<br>3. **POV 视角除错**：只要写了“第一人称 POV 视角”，就**绝对不可包含**人物外貌、神态或全身动作描写，只能用“单手/双手”、“镜头晃动”等主观第一人称状态。<br>4. **分镜图全面挂钩**：把生成的所有分镜图都通过 `@ImageN` 精确挂载到对应的名词上。<br>5. **摒弃 `@Audio` 标签** |
| 语言 | Prompt 使用**纯中文**；结尾附带极简角色状态和口播台词。对于 POV 视角，角色状态应转化为“镜头状态”（如：镜头急躁晃动）。 |

**输出格式**：

每段提示词直接放入 code block，**不加 `Prompt:` 标签**，方便用户一键复制：
...
<!-- 📋 COPYABLE OUTPUT -->
```text
第一人称 POV 视角，现代厨房，自然光。从 @图片1 拔出钝刀，像 @视频1 的姿势在 @图片2 番茄上按压。番茄变形切不开。她叹气道 "I used to struggle with this dull knife every single day, for real."
```
...
| 8 | **视频提示词结构** | @Video/@Image 逐行独立放置段末，已摒弃 @Audio？ |
| 9 | **中文翻译完整性** | 所有英文文案下方均附 `[中文翻译]: ...`？ |
| 10 | **分镜图提示词规范** | 6 宫格提示词遵循 5 段式公式？每格引用场景参考图？产品物理限制已写入？UGC 风格关键词到位？ |
| 11 | **可复制版本格式** | 纯净可复制版本是否为纯文本？脚本：无时间轴 `[0-3s]`、无引号；标签：仅一行 `#...`；视频/分镜提示词：无 `Prompt:` 标签，直接 code block？ |
| 12 | **安全合规检查** | 是否已全面彻底规避（政治敏感 / 暴力色情低俗 / 侵权身份伪造 / 广告法极端词），且**绝对没有包含任何危险/自残动作违禁词汇**？ |
